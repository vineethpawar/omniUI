...
```tsx
import { AppShell, Sidebar } from "@omniui/layouts";
...
The sidebar slot is a single render slot, so consumers can put the omniUI Sidebar there or any custom navigation surface.

## Native variant

On native the sidebar slot is dropped; tabs are the right pattern. Use `OmniNavigator mode="tabs"` from `@omniui/navigator/react-navigation` and feed it the same `OmniRoute[]` you'd render in the web Sidebar.

## Composing with the navigator

```tsx
import { defineRoutes } from "@omniui/navigator";
