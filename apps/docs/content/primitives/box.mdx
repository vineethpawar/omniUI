...
`Box` is the single most-reused primitive in omniUI. On the web it's polymorphic via the `as` prop, so it renders any HTML element you want while still typing the native attributes correctly. On native it's a `View` (RN has no polymorphism story worth simulating, so we don't pretend).
