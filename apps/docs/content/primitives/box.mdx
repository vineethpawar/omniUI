...
export default function App() {
  return (
    <Box surface="primary" padding="lg" style={{ minHeight: "100vh" }}>
      <Box surface="raised" padding="lg" radius="lg" style={{ maxWidth: 420, margin: "20px auto" }}>
        <Text size="xl" weight="bold">Hello from Box</Text>
        <Text size="sm" style={{ marginTop: 6, opacity: 0.7 }}>
          Edit anything on the left — the surface, padding, and radius props all flip live.
        </Text>
      </Box>
    </Box>
  );
