...
`Text` is omniUI's typography primitive. Sized via a small enum (`xs` to `3xl`), weighted via four named weights, color resolved against the theme, polymorphic on the element via the `as` prop on web.
